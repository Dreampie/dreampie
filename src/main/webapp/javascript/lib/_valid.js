...
(function ($, undefined) {
    ...
    function Valid($form, options) {
        ...
        this.options.boxer = $.extend({}, $.fn[pluginName].defaults.boxer, this.options.boxer);
        this.$form = $form;
        this.allowed_rules = [];
        this.errors = {};
        var self = this;
    ...
    $.extend(Valid.prototype, {
        ...
        showError: function (errors) {
            ...
            $.each(errors, function (k, v) {
                ...
                if (self.options.wrapper !== null) {
                    $input.closest(self.options.wrapper).addClass('error-boxer');
                } else {
                    $input.addClass('error-border');
                }
                if (!self.options.boxer.exist) {
                    var $boxer = $input.siblings('.error-box');

                    if ($boxer.length === 0) {
                        $boxer = $('<' + self.options.boxer.box + '/>', {'class': 'error-box'});
                        $input.after($boxer);
                    }
                    $boxer.html(v);
                } else {
                    var $boxer;
                    if (self.options.boxer.selector) {
                        $boxer = self.$form.find(self.options.boxer.selector);
                    } else
                        $boxer = self.$form.find('.error-box');
                    if ($boxer.html() === '') {
                        $boxer.append(v);
                    } else
                        $boxer.append(self.options.boxer.separator + v);
                }
            });
        },
        ...
        methods: {
            ...
            date_time: function (field, value) {
                var regex = /^(\d{4})(\/|-|\.|年)(\d{1,2})(\/|-|\.|月)(\d{1,2})(日)?(\s+\d{1,2}(:|时)\d{1,2}(:|分)?(\d{1,2}(秒)?)?)?$/;
                return regex.test(value);
            },
        ...
        return this.each(function (idx, form) {
            ...
            if ($.type(options) === 'object') {
                ...
                $form.ajaxForm({
                    ...
                    }, dataType: 'json', success: function (response, status, xhr, $form) {
                        if (response.status === 'fail') {
                            if (typeof options.onFail !== 'function' || options.onFail.call(this, response, validator, $form) !== false) {
                                validator.showError(response.data.errors);
                            }
    ...
    $.fn[pluginName].defaults = {
        wrapper: null 	// the html tag that wraps the field and which defines a "row" of the form use box
        , boxer: {box: 'span', exist: false, separator: '<br/>', selector: null} //messages box
        ...
        , messages: {}		// custom error messages like {yusername: {not_empt: 'hey you forgot to enter your username', min_length: 'come on, more than 2 chars, ok?'}, password: {....}}
    };
})(jQuery);
