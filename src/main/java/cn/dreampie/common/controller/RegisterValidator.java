...
public class RegisterValidator extends Validator {
  protected void validate(Controller c) {
    ...
    if (!usernameEmpty && !ValidateUtils.me().isUsername(c.getPara("user.username"))) addError("usernameMsg", "用户名为5-18为字母,数字和下划线的组合");
    ...
    if (!lastnameEmpty && c.getPara("user.last_name").length() > 10) addError("lastnameMsg", "名字不能多于10位");


    boolean passwordEmpty = ValidateUtils.me().isNullOrEmpty(c.getPara("user.password"));
    if (passwordEmpty) addError("passwordMsg", "请输入密码");
    if (!passwordEmpty && !ValidateUtils.me().isPassword(c.getPara("user.password"))) addError("passwordMsg", "密码为5-18为字母,数字和下划线的组合");
    ...
    if (!passwordEmpty && !repasswordEmpty && !c.getPara("user.password").equals(c.getPara("repassword"))) addError("repasswordMsg", "重复密码不一致");
